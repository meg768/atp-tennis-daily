...
After `Odds`, render a `Kelly` block. Show one short intro line before the Kelly table: `Förslag på bets enligt Kelly-modellen. PlayerA mot PlayerB.` Assume a fixed bankroll of 1000 kr. Use Svenska Spel as the stakeable bookmaker odds and Tennis Abstract or Vitel as the model-implied probability. In the `Spel (%ROI)` column, show player, Svenska Spel odds, and ROI inline, for example `Ethan Quinn @ 2.25 (+8% ROI)`, where ROI is `(bookmaker_odds * p - 1) * 100` rounded to whole percentages. Calculate the full Kelly base as `((odds - 1) * p - (1 - p)) / (odds - 1) * 1000`, but show only 1/2 Kelly, 1/4 Kelly, and 1/8 Kelly in whole kronor. Omit or show `0 kr` for non-positive Kelly signals. Always keep the Swedish bankroll note under the Kelly table.
...
Use the named block structure from `template.html` consistently rather than inventing ad-hoc wrappers per match. The intended order is a full-width `match-block--odds`, then one single broad column with `match-block--kelly`, `match-block--play`, `match-block--form`, `match-block--win-rate`, `match-block--recent-results`, `match-block--head-to-head`, `match-block--status`, `match-block--market`, `match-block--decider`.
