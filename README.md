...
After `Odds`, render a `Kelly` block. Show one short intro line before the Kelly table: `Förslag på bets enligt Kelly-modellen. PlayerA mot PlayerB`. Assume a fixed bankroll of 1000 kr. Use Svenska Spel as the stakeable bookmaker odds and Tennis Abstract or Vitel as the model-implied probability. In the `Spel (%ROI)` column, show player, Svenska Spel odds, and ROI inline, for example `Ethan Quinn @ 2.25 (+8% ROI)`, where ROI is `(bookmaker_odds * p - 1) * 100` rounded to whole percentages. Calculate the full Kelly base as `((odds - 1) * p - (1 - p)) / (odds - 1) * 1000`, but show only 1/2 Kelly, 1/4 Kelly, and 1/8 Kelly in whole kronor. Omit or show `0 kr` for non-positive Kelly signals.
...
Use the named block structure from `template.html` consistently rather than inventing ad-hoc wrappers per match. The intended order is a full-width `match-block--odds`, then `match-block--kelly`, then one single broad column with `match-block--play`, `match-block--form`, `match-block--win-rate`, `match-block--recent-results`, `match-block--head-to-head`, `match-block--status`, `match-block--market`, `match-block--decider`.
For table slots such as `odds-table`, `kelly-table`, `win-rate-table`, recent-results tables, and head-to-head, fill the existing table with `thead` and `tbody` content rather than inserting a complete nested `<table>` inside the slotted table. For paragraph slots, replace the placeholder text rather than appending generated prose before it.
Before the match title, render one compact kicker line with start time and tournament/round context.
Before the metadata cards and deeper match blocks, keep one compact top overview row with `Snabbkoll`, `Matchprofil`, and `Dagsläge` so the page orients the reader before the long-form analysis starts. Preserve each overview card as a `.match-overview__item` with a `.match-overview__label` followed by its value; do not collapse the label and value into plain inline text.
In the metadata cards, prefer one compact `Nyckelsvar` card over a plain `Tid` card; it should give an answer, not ask a question.
