...
After `Odds`, render a `Kelly` block. Assume a fixed bankroll of 1000 kr. Use Svenska Spel as the stakeable bookmaker odds and Tennis Abstract or Vitel as the model-implied probability. Calculate the full Kelly base as `((odds - 1) * p - (1 - p)) / (odds - 1) * 1000`, but show only 1/2 Kelly, 1/4 Kelly, and 1/8 Kelly in whole kronor. Omit or show `0 kr` for non-positive Kelly signals.
