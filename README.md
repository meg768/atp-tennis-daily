...
In `Senaste resultat`, keep the structure fixed to exactly two columns, `Datum` and `Resultat`, with natural-language rows such as `Musetti vann mot Etcheverry 6-4 6-3`. In visible date columns such as `Head-to-head` and `Senaste resultat`, render `YYYY-MM-DD` rather than full ISO timestamps.
